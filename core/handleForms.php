...
if (isset($_POST['createAlbumBtn'])) {
    ...
    if (!empty($album_name)) {
        if (createAlbum($pdo, $album_name, $username)) {
...
if (isset($_POST['updateAlbumBtn'])) {
    ...
    if (updateAlbumName($pdo, $album_id, $new_name)) {
        ...
        $_SESSION['message'] = "Error updating album.";
    }
    header("Location: ../albums.php");
}
...
if (isset($_POST['deleteAlbumBtn'])) {
    ...
    if (deleteAlbum($pdo, $album_id)) {
        ...
        $_SESSION['message'] = "Error deleting album.";
    }
    header("Location: ../albums.php");
}

if (isset($_POST['addPhotoToAlbumBtn'])) {
    $album_id = $_POST['album_id'];
    ...
    $folder = "../images/" . $photoName;

    if (move_uploaded_file($tempFileName, $folder)) {
        if (addPhotoToAlbum($pdo, $photoName, $album_id, $description, $username)) {
            $_SESSION['message'] = "Photo added to album successfully!";
        } else {
            $_SESSION['message'] = "Error adding photo to album.";
        }
    } else {
