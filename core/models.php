...
function addPhotoToAlbum($pdo, $photo_name, $album_id, $description, $username) {
    $sql = "INSERT INTO photos (photo_name, album_id, description, username) VALUES (?, ?, ?, ?)";
    $stmt = $pdo->prepare($sql);
    return $stmt->execute([$photo_name, $album_id, $description, $username]);
}
